...
/**
 * POST /api/admin/payments/phonepe/create-order
 * Tenant-scoped: only the authenticated org (tenant) can create a payment for themselves.
 * Body: { amountPaisa: number, description?: string, plan?: string }
 * Returns: { redirectUrl, merchantOrderId, orderId }. Redirect user to redirectUrl to pay.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    const description = typeof body.description === "string" ? body.description.slice(0, 200) : "Subscription payment";
    ...
    console.error("[admin/payments/phonepe/create-order]", e);
    return NextResponse.json(
